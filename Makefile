...
lint:
	...
	@$(DOCKER) run -t --rm -e GOEXPERIMENT=jsonv2 -v $(PWD):/app:rw,z -w /app golangci/golangci-lint:v2.12.2 golangci-lint run -v

lint-fix:
	@echo "Running golangci-lint run --fix"
	@$(DOCKER) run -t --rm -e GOEXPERIMENT=jsonv2 -v $(PWD):/app:rw,z -w /app golangci/golangci-lint:v2.12.2 golangci-lint run --fix -v
